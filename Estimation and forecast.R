...
NA_GARCH_Evaluation <- function(r, P_t, N_t, a, b, kappa, gamma, split_ratio, params) {
  ...
  log_likelihood <- function(r, P_t, N_t, a, b, kappa, gamma, split_ratio, params) {
    ...
    for (t in 2:n) {
      f[t] <- a + 0.5 * b * ((exp(kappa * P_t_in[t-1]) - 1) / (exp(kappa * P_t_in[t-1]) + 1) - (exp(gamma * N_t_in[t-1]) - 1) / (exp(gamma * N_t_in[t-1]) + 1))
      #NA-GARCH
  ...
  logL <- fit$value
  k <- length(est_params)
  AIC <- if(is.numeric(logL)) {2 * k - 2 * logL} else {"Do not converge"}
  n_out <- length(r_out)
